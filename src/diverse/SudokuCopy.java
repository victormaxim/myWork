...
/**
 * The <code>diverse.Sudoku</code> class povides a static <code>main</code>
 * method allowing it to be called from the command line to print the
 * solution to a specified diverse.Sudoku problem.
 *
 * <p>The following is an example of a diverse.Sudoku problem:
 *
 * <pre>
 * -----------------------
 * |   8   | 4   2 |   6   |
 ...
 * 
 * <pre>
 * % java -cp . diverse.Sudoku 018 034 052 076 \
 *                     113 124 169 171 \
 *                     209 216 278 284 \
 *                     332 341 356     \
 *                     533 545 557     \
 ...
 * diverse.Sudoku</a> for more information on diverse.Sudoku.
 *
 ...
 */
public class SudokuCopy {

	static int triess;
    /**
     * Print the specified diverse.Sudoku problem and its solution.  The
	...
	public static void main(String[] args) {
		...
		//afiseaza matricea inca nerezolvata
    	writeMatrix(matrix);
    	//triess++;
    	//int tries;
    	//Array matrice = new Array();
    ...
    if (solve(0,0,matrix)){    // solves in place
	    ...
	    System.out.println("That took "+(timeEnd-timeStart)/1000+" seconds to complete." + triess);
    }
	else 
	...
	for (int val = 1; val <= 9; ++val) {
		...
		if (solve(i+1,j,cells))  {
			System.out.println(++i + "-" + ++j + " (" + triess + "): " + val);
			return true;	}
	    }
	}
	...
	for (int k = 0; k < 3; ++k) // box
	    for (int m = 0; m < 3; ++m)
		if (val == cells[boxRowOffset+k][boxColOffset+m])return false;
